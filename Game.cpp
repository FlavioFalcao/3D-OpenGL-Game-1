...
int myTimer; // Timer for the Game
...
bool upkey=0,downkey=0,leftkey=0,rightkey=0, zgkey=0; // ? handleKeypress shoud take care of all this
...
void DrawGateway(void){
	...
	Location temp=Level_Index[currLevel].Position[(int)Level_Index[currLevel].Position.size()-1];
	glBegin(GL_QUADS);
...
void HUD(int score, int time, int strength) {
	...
	renderBitmapString((w-windowWidth)/2,50,(void *)font,"Total Time Elapsed");
	renderBitmapString((w-windowWidth)/2+HUD_SHIFT,50, (void*)font, a);
	
	renderBitmapString((w-windowWidth)/2,80,(void *)font,"Level Score :- ");
	renderBitmapString((w-windowWidth)/2+HUD_SHIFT,80,(void *)font,b);
	
	renderBitmapString((w-windowWidth)/2,110,(void *)font,"Final Score :- ");
	renderBitmapString((w-windowWidth)/2+HUD_SHIFT,110,(void *)font,c);

	renderBitmapString((w-windowWidth)/2,140,(void *)font,"Tile Strength:- ");
	renderStrengthBar((w-windowWidth)/2+HUD_SHIFT,140,strength*5);
	
	renderBitmapString((w-windowWidth)/2,170,(void *)font,"Zero Gravity:- ");
	renderStrengthBar((w-windowWidth)/2+HUD_SHIFT,170,zgtime);
...
void initSkybox(void){
	...
	addTexture(skyboxTex[0], "1_front.jpg");
	addTexture(skyboxTex[0], "1_left.jpg");
	addTexture(skyboxTex[0], "1_back.jpg");
	addTexture(skyboxTex[0], "1_right.jpg");
	addTexture(skyboxTex[0], "1_top.jpg");
	addTexture(skyboxTex[0], "1_bot.jpg");

	addTexture(skyboxTex[1], "2_front.jpg");
	addTexture(skyboxTex[1], "2_left.jpg");
	addTexture(skyboxTex[1], "2_back.jpg");
	addTexture(skyboxTex[1], "2_right.jpg");
	addTexture(skyboxTex[1], "2_top.jpg");
	addTexture(skyboxTex[1], "2_bot.jpg");

	addTexture(skyboxTex[2], "3_front.jpg");
	addTexture(skyboxTex[2], "3_left.jpg");
	addTexture(skyboxTex[2], "3_back.jpg");
	addTexture(skyboxTex[2], "3_right.jpg");
	addTexture(skyboxTex[2], "3_top.jpg");
	addTexture(skyboxTex[2], "3_bot.jpg");
...
void skyboxdraw(void) {
   ...
   // Just in case we set all vertices to white.
    ...
    glBindTexture(GL_TEXTURE_2D, skyboxTex[currLevel][0]);
    glBegin(GL_QUADS);
    ...
    glBindTexture(GL_TEXTURE_2D, skyboxTex[currLevel][1]);
    glBegin(GL_QUADS);
        glTexCoord2f(0, 0); glVertex3f(  INF, -INF,  INF );
        glTexCoord2f(1, 0); glVertex3f(  INF, -INF, -INF );
        glTexCoord2f(1, 1); glVertex3f(  INF,  INF, -INF );
    ...
    glBindTexture(GL_TEXTURE_2D, skyboxTex[currLevel][2]);
    glBegin(GL_QUADS);
        glTexCoord2f(0, 0); glVertex3f( -INF, -INF,  INF );
        glTexCoord2f(1, 0); glVertex3f(  INF, -INF,  INF );
        glTexCoord2f(1, 1); glVertex3f(  INF,  INF,  INF );
    ...
    glBindTexture(GL_TEXTURE_2D, skyboxTex[currLevel][3]);
    glBegin(GL_QUADS);
        glTexCoord2f(0, 0); glVertex3f( -INF, -INF, -INF );
        glTexCoord2f(1, 0); glVertex3f( -INF, -INF,  INF );
        glTexCoord2f(1, 1); glVertex3f( -INF,  INF,  INF );
    ...
    // Render the top quad
    glBindTexture(GL_TEXTURE_2D, skyboxTex[currLevel][4]);
    glBegin(GL_QUADS);
        glTexCoord2f(0, 1); glVertex3f( -INF,  INF, -INF );
        glTexCoord2f(0, 0); glVertex3f( -INF,  INF,  INF );
        glTexCoord2f(1, 0); glVertex3f(  INF,  INF,  INF );
    ...
    glBindTexture(GL_TEXTURE_2D, skyboxTex[currLevel][5]);
    glBegin(GL_QUADS);
        glTexCoord2f(0, 0); glVertex3f( -INF, -INF, -INF );
        glTexCoord2f(0, 1); glVertex3f( -INF, -INF,  INF );
        glTexCoord2f(1, 1); glVertex3f(  INF, -INF,  INF );
...
void moveVader(float X){
		Location NewPos;
		NewPos.y = myChar.At.y;
		NewPos.x = myChar.At.x + (X) * sin(myChar.viewangle * pi/180.0);
		NewPos.z = myChar.At.z + (X) * cos(myChar.viewangle * pi/180.0);
		
		unsigned int i;
		for( i=0;i<myTiles.size();++i ){
			if( myTiles[i].dead )
				continue;
			if( ((NewPos.x-myTiles[i].At.x)>=-0.3 && ((myTiles[i].At.x+2.0)-NewPos.x)>=-0.3)){
				if( ((NewPos.z-myTiles[i].At.z)>=-0.3 && ((myTiles[i].At.z+2.0)-NewPos.z)>=-0.3) )
					break;
			}
		}
		...
		if( i<myTiles.size() ){
			...
			if( myTiles[i].box ){
				if(!(((myChar.At.y-1.0)-(myTiles[i].At.y+2.0))>=0.0) )
					return;
			}
			else{
				if( !(((myChar.At.y-1.0)-(myTiles[i].At.y))>=0.0) )
					return;
			}
		}
...
void fixCamera(){
	if( ckey%6==1 ){
		myCam = Cam( Location( myChar.At.x, myChar.At.y+1.0, myChar.At.z-10.5), 
			Location(myChar.At.x + 0.1f * sin(myChar.viewangle * pi/180.0), myChar.At.y + 0.97, myChar.At.z -10.5f + 0.1f * cos(myChar.viewangle * pi/180.0)),
			Location(0, 1, 0));
	}
	else if( ckey%6==2 ){
		myCam = Cam( Location(myChar.At.x, 10.0f, myChar.At.z-10.0f), 
			Location( myChar.At.x, 0.0f, myChar.At.z-10.0f),
...
void drawScene() {
	...
	for( i=0;i<myTiles.size();++i ){
		if( myTiles[i].dead )
			continue;
		if( ((myChar.At.x-myTiles[i].At.x)>=0.0 && ((myTiles[i].At.x+2.0)-myChar.At.x)>=0.0)){
			if( ((myChar.At.z-myTiles[i].At.z)>=0.0 && ((myTiles[i].At.z+2.0)-myChar.At.z)>=0.0))
				break;
		}
	}

	if(i<myTiles.size())
		HUD(myChar.Score, myTimer/20, myTiles[i].strength);
	else
		HUD(myChar.Score, myTimer/20, 0);
	/*********************************************************/
	...
	for( size_t i=0;i<myTiles.size()-1; ++i ){
		myTiles[i].draw();
	}
	/*******************************************************/
	DrawGateway();
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Save Darth Vader");
	initRendering();
